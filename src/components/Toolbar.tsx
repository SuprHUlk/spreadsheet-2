...
import { ToolbarProps, Formula } from '../types';
  ...
  return (
    <div className="toolbar bg-white p-1 flex items-center gap-1 border-b border-gray-300">
      ...
      <select 
        ...
        value={currentFont}
        onChange={(e: ChangeEvent<HTMLSelectElement>) => onFontFamily(e.target.value)}
      >
      ...
      <button 
        ...
        onClick={() => {
          const currentSize = parseInt(cellStyles[selectedCell]?.fontSize || '10');
      ...
      <input 
        ...
        value={parseInt(cellStyles[selectedCell]?.fontSize || '10')}
      ...
      <button 
        className={`toolbar-btn ${cellStyles[selectedCell]?.bold ? 'bg-gray-200' : ''}`}
        onClick={onBold}
        title="Bold (Ctrl+B)"
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
      ...
      <button 
        className={`toolbar-btn ${cellStyles[selectedCell]?.italic ? 'bg-gray-200' : ''}`}
        onClick={onItalic}
        title="Italic (Ctrl+I)"
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
      ...
      <input 
        ...
        onChange={(e: ChangeEvent<HTMLInputElement>) => onColor(e.target.value)}
        value={cellStyles[selectedCell]?.color || '#000000'}
        title="Text color"
      />
      ...
      <div className="flex items-center align-btns">
        <button 
          className={`toolbar-btn ${cellStyles[selectedCell]?.textAlign === 'left' ? 'bg-gray-200' : ''}`}
        ...
        <button 
          className={`toolbar-btn ${cellStyles[selectedCell]?.textAlign === 'center' ? 'bg-gray-200' : ''}`}
        ...
        <button 
          className={`toolbar-btn ${cellStyles[selectedCell]?.textAlign === 'right' ? 'bg-gray-200' : ''}`}
      ...
      <select 
        ...
        onChange={(e: ChangeEvent<HTMLSelectElement>) => onFormulaSelect(e.target.value)}
        value=""
      >
